...
def prepare_input_for_ernierna(index,seq_len,contact_h5=None, seq_id=None):
    
    shorten_index = index[:seq_len+2]
    one_d = torch.from_numpy(shorten_index).long().reshape(1,-1)
    two_d = np.zeros((1, seq_len+2, seq_len+2), dtype=np.float32)

    #因为用于预测的序列较少，所以一个H5文件就足够了，不需要映射。

    #加入支持H5文件的加载，用于取代对碱基互补配对的计算调用
    if contact_h5 is not None and seq_id is not None:
        try:
            with h5py.File(contact_h5, 'r') as f:
                if seq_id in f:
                    contact = f[seq_id][:]  # shape (seq_len, seq_len)
                else:
                    raise KeyError(f"Sequence ID '{seq_id}' not found in HDF5 file")
        except Exception as e:
            print(f"Warning: Failed to load contact map for {seq_id}: {e}. Falling back to creatmat.")
            contact = None

        if contact is not None:
            if contact.shape[0] == seq_len:
                matrix = contact
            elif contact.shape[0] == seq_len + 2:
                matrix = contact[1:-1, 1:-1]   # 去掉首尾特殊token对应的行列
            else:
                raise ValueError(f"Contact map size {contact.shape[0]} != sequence length {seq_len} or {seq_len+2}")
            # 将接触矩阵放入第一个切片，嵌入到特殊token之间（BOS和EOS位置为0）
            two_d[0, 1:1+seq_len, 1:1+seq_len] = matrix
        else:
            # 回退到碱基配对规则
    ...
    # 转置为 (L+2, L+2, 1) 并 reshape 为 (1, L+2, L+2, 1)
    two_d = two_d.transpose(1, 2, 0)  # (1, L, L) -> (L, L, 1)
...
def load_pretrained_ernierna(mlm_pretrained_model_path, arg_overrides):
    if isinstance(arg_overrides, str):
        arg_overrides = json.loads(arg_overrides)
    elif arg_overrides is None:
        arg_overrides = {}
    rna_models, _, _ = checkpoint_utils.load_model_ensemble_and_task(
        mlm_pretrained_model_path.split(os.pathsep),

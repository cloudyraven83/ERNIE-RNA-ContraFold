...
def post_process(u, x, lr_min, lr_max, num_itr, rho=0.0, with_l1=False,s=None): #s=math.log(9.0)
    """
...
#加入contact_h5变量
def predict(rna_lst, seq_names, best_model_path=None, mlm_pretrained_model_path=None, arg_overrides=None, device='cpu', save_path='./results/',contact_h5=None):
    '''
    input:
    rna_lst: List of rna str
    ...
    '''
    # 确保 CT 文件输出目录存在
    finetune_dir = os.path.join(save_path, 'finetune_prediction')
    zeroshot_dir = os.path.join(save_path, 'zeroshot_prediction')
    for d in [finetune_dir, zeroshot_dir]:
    ...
    with torch.no_grad():
        ...
        for i, (seq, seq_name) in enumerate(zip(rna_lst, seq_names)):
            try:  ##错误处理，如果一个序列ID找不到对应H5文件矩阵，则跳过，不会中断处理
                X, data_seq = seq_to_rnaindex_and_onehot(seq)
                # 原始ID用于HDF5查找（取第一个空格和第一个|之前）
                seq_id = seq_name.split()[0].split('|')[0]
                # 生成规范文件名
                safe_seq_id = re.sub(r'[^a-zA-Z0-9_]', '_', seq_id)
                if len(safe_seq_id) > 100:
                    safe_seq_id = safe_seq_id[:100]
                #确保序列ID能够对应H5文件内的key，同时确保生成的文件名合乎规范

                #断点重传，调试时使用，的检查预测文件是否已存在
                zeroshot_file = os.path.join(zeroshot_dir, f"{safe_seq_id}_zeroshot_prediction.ct")
                if os.path.exists(zeroshot_file):
                    print(f"Skipping {seq_name} (already predicted)")
                    # 虽然跳过，但仍记录该文件路径
                    ct_files_lst.append(zeroshot_file)
                    continue

                one_d, twod_data = prepare_input_for_ernierna(X, len(seq), contact_h5=contact_h5, seq_id=seq_id)
                ...
                if my_model is not None:
                    ...
                    pair_attn = pred_ss
                    finetune_ct_file = post_process_prediction(pair_attn, data_seq, seq,
                                                               f"{safe_seq_id}_finetune_prediction", finetune_dir)
                ...
                attn_all = torch.stack(pred_ss)  # (num_layers, B, num_heads, T, T)
                attn_avg = attn_all.mean(dim=(0, 2))  # (B, T, T) 平均所有层和头
                test_attn = attn_avg[0, 1:-1, 1:-1]   # 去掉特殊 token
                pair_attn = (test_attn + test_attn.T) / 2
                zeroshot_ct_file = post_process_prediction(pair_attn, data_seq, seq,
                                                           f"{safe_seq_id}_zeroshot_prediction", zeroshot_dir)
                ct_files_lst.append(zeroshot_ct_file)
                del pair_attn, pred_ss

                print(f"Processed sequence {i+1}/{total}: {seq_name}")

            except Exception as e:
                print(f"Error processing {seq_name} (idx {i+1}): {e}")
                continue   # 继续下一个序列
...
if __name__ == '__main__':
    ...
    #添加命令行参数
    parser.add_argument("--contact_h5", default=None, type=str,
                        help="Path to HDF5 file containing precomputed contact maps (e.g., from ContraFold). "
                             "If not provided, base-pairing rules will be used.")
    parser.add_argument("--seqs_path", default='./data/ss_prediction/rna3db_testseqs.fasta', type=str,
                        help="The path of input seqs")
    parser.add_argument("--save_path", default='./results/ernie_rna_ss_prediction/rna3db_test_results/', type=str,
                        help="The path of rna ss extracted by ERNIE-RNA")
    parser.add_argument("--arg_overrides", default={"data": './src/dict/'}, help="The path of vocabulary")
    parser.add_argument("--ernie_rna_pretrained_checkpoint", default='./checkpoint/ERNIE-RNA_checkpoint/ERNIE-RNA_pretrain.pt',
                        type=str, help="The path of pre-trained ERNIE-RNA checkpoint")
    parser.add_argument("--dataset_name", default=None, type=str,
                        help="Dataset name (bpRNA-1m, RNAStralign, RIVAS, RNA3DB, bpRNA-new, bpRNA-1m_RNAstralign). "
                             "If specified, ss_rna_checkpoint will be auto-set.")
    parser.add_argument("--ss_rna_checkpoint", default=None, type=str,
                        help="The path of fine-tuned ERNIE-RNA checkpoint")
    ...
    # 如果既没有 dataset_name 也没有 ss_rna_checkpoint，则保持 ss_rna_checkpoint = None（仅零样本）
    # 不需要额外操作，args.ss_rna_checkpoint 已经是 None
    ...
    # 预测RNA二级结构并生成CT文件
    ct_files = predict(seqs_lst, seq_names, args.ss_rna_checkpoint,
                       args.ernie_rna_pretrained_checkpoint,
                       args.arg_overrides, args.device, args.save_path,
                       contact_h5=args.contact_h5)
